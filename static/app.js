...
function updateNextEvent() {
  const link=$('next-event-link');
  if(!link) return;
  const now=Date.now();
  const today=new Date().toISOString().slice(0,10);
  const next=data.upcoming.find(f=>{
    const state=f.odds?.state;
    const start=Date.parse(f.odds?.start_at || f.start_at);
    return state!=='post' && state!=='in' && (Number.isFinite(start)?start>now:f.date>=today);
  });
  link.dataset.event=next?`${next.date}|${next.event}`:'';
  $('next-event-label').textContent=next?'NEXT EVENT':'EXPLORE';
  $('next-event-date').textContent=next?new Date(`${next.date}T12:00:00`).toLocaleDateString(undefined,{month:'short',day:'numeric'}):'Fight cards';
  link.title=next?`${next.event} - View fight card`:'Browse upcoming fight cards';
  link.setAttribute('aria-label',next?`View ${next.event} on ${day(next.date)}`:'Browse upcoming fight cards');
}
$('next-event-link')?.addEventListener('click',()=>{
  showTab('upcoming');
  const event=$('next-event-link').dataset.event;
  if(data){if(event)$('event-filter').value=event;render();}
});
function render() {
  const m = data.model;
  if(m) {
    const boosted=m.name.startsWith('Gradient boosting');
    const estimator=boosted?'HistGradientBoostingClassifier':'LogisticRegression';
    const displayName=boosted?'Gradient boosting':'Logistic regression';
    $('model-summary').innerHTML = `<span class="model-overview">We predict fight outcomes from fighter records and past performance, selecting between <b class="glow-rose">logistic regression</b> and <b class="glow-violet">gradient-boosted trees</b>.</span><span class="model-selection"><span class="model-kicker">MODEL USED</span><strong class="active-model">${displayName}</strong><span class="model-estimator">scikit-learn ${estimator}</span></span><span class="model-training">Trained on <strong class="glow-blue">${m.metrics.training_fights.toLocaleString()} confirmed UFC results</strong>.</span><span class="model-inputs compact-inputs" role="list" aria-label="Training inputs"><span role="listitem">Wins, losses &amp; experience</span><span role="listitem">Opponent-adjusted Elo</span><span role="listitem">Recent form &amp; streaks</span><span role="listitem">KO &amp; submission rates</span><span role="listitem">Time since last fight</span></span>`;
  }
