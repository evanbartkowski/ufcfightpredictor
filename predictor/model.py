...
def fresh():
    return dict(elo=1500., wins=0, losses=0, streak=0, recent=[], ko=0, sub=0, last=None)
...
def vector(state, day):
    ...
    return np.array([state['elo'], state['wins'], state['losses'], state['streak'],
                     ...
                     state['ko'] / max(n, 1), state['sub'] / max(n, 1), n, rest], dtype=float)
...
def train(fights):
    ...
    factories = {
        'Logistic regression': lambda: make_pipeline(StandardScaler(), LogisticRegression(C=.1, max_iter=1000)),
        'Gradient boosting': lambda: HistGradientBoostingClassifier(max_iter=130, max_leaf_nodes=7,
            learning_rate=.045, l2_regularization=8, early_stopping=False, random_state=42),
    }
    scores = {}
    for name, factory in factories.items():
        candidate = fit(factory(), x[tr], y[tr])
        scores[name] = float(log_loss(y[va], probability(candidate, x[va]), labels=[0, 1]))
    ...
    metrics = dict(accuracy=float(accuracy_score(y[te], p >= .5)),
                   ...
                   training_fights=len(examples), validation_log_loss=scores)
