...
Event dates are split into 65% training, 15% selection, and 20% held-out testing. Logistic regression and gradient boosting compete on validation log loss. The selected approach is refitted on the first 80% and evaluated on the final 20%; the serving model is then fitted on all confirmed results. Preprocessing is fitted only on the applicable training partition. Holdout accuracy, Brier score, log loss, and Elo/majority baselines are shown separately from live prediction accuracy. Evaluation is recomputed when data changes, so successive holdout scores are not a fixed benchmark.
